...
export class LuckySheet extends LuckySheetBase {
    ...
    constructor(sheetName:string, sheetId:string, sheetOrder:number,isInitialCell:boolean=false, allFileOption:any){
        ...
        this.definedNames = allFileOption.definedNames;
        this.sheets = allFileOption.sheets;
        ...
        if(sheetView.length>0){
            ...
            if(panes != null && panes.length > 0) {
                let attributeList = panes[0].attributeList;
                let type = '', range: {};
                let ySplit = (parseFloat(attributeList.ySplit) || 0) - 1;
                let xSplit = (parseFloat(attributeList.xSplit) || 0) - 1;
                if(attributeList.xSplit && attributeList.ySplit && ySplit >= 0 && xSplit >= 0) {
                    type = 'rangeBoth',
                    range = { row_focus: ySplit, column_focus: xSplit }
                }
                else if(attributeList.ySplit && ySplit >= 0) {
                    type = 'rangeRow',
                    range = { row_focus: ySplit, column_focus: 0 }
                }
                else if(attributeList.xSplit && xSplit >= 0) {
                    type = 'rangeColumn',
                    range = { row_focus: 0, column_focus: xSplit }
                }
                if(type) {
                    this.frozen = { type: type, range: range }
                }
            }
        }
        ...
        // rangeNames config
        this.rangeNames = this.generateConfigRangeNames();

        // conditionFormat config
        this.luckysheet_conditionformat_save = this.generateConfigConditionformat();
    ...
    */
        ...
        for(let i=0;i<rows.length;i++){
            ...
            if(this.isInitialCell){
                ...
                for(let key in cells){
                    let cell = cells[key];
                    let cellValue = this.getCellValue(cell, this.sheetFile, cellOtherInfo)
    ...
    getCellValue(cell: Element, sheetFile: string, cellOtherInfo?: IcellOtherInfo) {
    ...
    getSheetBysheetName(name: string) {
        let sheet = this.sheets.find(s => s.attributeList.name === name);
        if(!sheet) return
        return sheet
    }

    getMulitValue(formulaValueArr: string[]) {
        let valueArr: string[] = []
        for (const key in formulaValueArr) {
            const item = formulaValueArr[key]
    ...
    private generateConfigDataValidations(): IluckysheetDataVerification {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (formulaReg.test(formulaValue) && extLst?.length >= 0) {
          ...
          formulaValueArr = getMultiFormulaValue(peelOffData?.formula);
          valueArr = this.getMulitValue(formulaValueArr)
        } else {
          operator = getXmlAttibute(attrList, "operator", null);
          sqref = getXmlAttibute(attrList, "sqref", null);
          sqrefIndexArr = getMultiSequenceToNum(sqref);
          formulaValueArr = getMultiFormulaValue(formulaValue);
          valueArr = this.getMulitValue(formulaValueArr)
        }
    ...
    private generateConfigRangeNames(): IluckysheetRangeName[] {
        const rangeNames:IluckysheetRangeName[] = []
        for (const item of this.definedNames) {
            const attrList = item.attributeList
            const valueArr = item.value.split("!")
            const order = Number(this.sheetList[valueArr[0]]) - 1
            if(Number(this.order) !== order) continue
            const range = valueArr[1].replaceAll('$', '')
            const name:IluckysheetRangeName = {
                cellImageId: null,
                name: attrList.name,
                order: order,
                range: range
    ...
    private generateConfigConditionformat(): IluckysheetConditionFormat[] {
        const conditionFormats:IluckysheetConditionFormat[] = []
        for (const item of this.definedNames) {
            const attrList = item.attributeList
            const valueArr = item.value.split("!")
            const order = Number(this.sheetList[valueArr[0]]) - 1
            if(Number(this.order) !== order) continue
            const sheet = this.getSheetBysheetName(valueArr[0])
            const sheetId = sheet.attributeList["r:id"];
            const cellrange = getcellrange(item.value, this.sheetList, sheetId)
            const conditionFormat:IluckysheetConditionFormat = {
                cellImageId: null,
